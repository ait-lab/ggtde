...
class IV_PPO(PPO):
    def value_loss_fn(self, values: th.Tensor, returns: th.Tensor) -> th.Tensor:
        return mse_loss(values, returns) + self.uncertainty_temperature * biv_loss(
            values, returns, self.gamma, self.min_batch_size
        )


class IEV_PPO(PPO):
    def value_loss_fn(self, values: th.Tensor, returns: th.Tensor) -> th.Tensor:
        return mse_loss(values, returns) + self.uncertainty_temperature * biev_loss(
            values, returns, self.min_batch_size
        )
