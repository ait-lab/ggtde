...
def weighted_mse_loss(
    values: th.Tensor, targets: th.Tensor, weights: th.Tensor
) -> th.Tensor:
    ...
    loss = sum(
        F.mse_loss(values[:, i, 0], targets[:, i], reduction="none") * weights
...
def biv_loss(
    values: th.Tensor, targets: th.Tensor, gamma: float, min_batch_size: int
) -> th.Tensor:
    ...
    return weighted_mse_loss(values, targets, biv_weights)


def biev_loss(values: th.Tensor, targets: th.Tensor, min_batch_size: int) -> th.Tensor:
    ...
    return weighted_mse_loss(values, targets, biev_weights)
